...
class MockStageManager:
    ...
    def _ensure_directories(self) -> None:
        ...
        for path in [self.base_path, self.stages_path, self.tables_path, 
                     self.user_path, self.external_path]:
    ...
    def create_stage(self, name: str, stage_type: str = "USER", 
                    url: Optional[str] = None, properties: Optional[Dict[str, Any]] = None) -> Stage:
        """Create a new stage."""
    ...
    def _determine_local_path(self, name: str, stage_type: str, url: Optional[str]) -> str:
        """Determine local path for a stage based on type and URL."""
        if stage_type == "USER":
            return str(self.user_path / name)
        elif stage_type == "INTERNAL" or stage_type == "TABLE":
            return str(self.tables_path / name)
        elif stage_type == "EXTERNAL":
            if url:
    ...
    def _store_stage_metadata(self, stage: Stage) -> None:
        ...
        self.connection.execute(insert_sql, [
            ...
            stage.url,
            stage.local_path,
            json.dumps(stage.properties)
        ])

    def get_stage(self, name: str) -> Optional[Stage]:
        """Get stage by name."""
        result = self.connection.execute(
            "SELECT * FROM mockhaus_stages WHERE name = ?", [name]
        ).fetchone()
    ...
    def resolve_stage_path(self, stage_reference: str) -> Optional[str]:
        """
    ...
    def ensure_stage_directory(self, stage_reference: str) -> str:
        ...
        if not path.suffix == '' or '.' in path.name:
