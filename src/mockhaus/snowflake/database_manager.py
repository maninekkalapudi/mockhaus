...
class SnowflakeDatabaseManager:
    ...
    def is_database_ddl(self, sql: str) -> bool:
        ...
        return (
            ...
            sql_upper.startswith("USE DATABASE") or
            sql_upper.startswith("USE ") or
            sql_upper.startswith("SHOW DATABASES")
        )
    ...
    def execute_database_ddl(self, sql: str) -> dict:
        ...
        if sql_upper.startswith("CREATE DATABASE"):
            ...
            return {
                "success": False,
                "error": f"Unsupported database DDL: {sql}"
            }
    ...
    def _create_database(self, sql: str) -> dict:
        ...
        if not match:
            return {
                "success": False,
                "error": "Invalid CREATE DATABASE syntax. Use: CREATE DATABASE database_name"
            }
        ...
        if db_file.exists() and not if_not_exists:
            return {
                "success": False,
                "error": f"Database '{db_name_raw}' already exists"
            }
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"Database '{db_name_raw}' created successfully",
                "database_path": str(db_file)
            }
        except Exception as e:
            return {
                "success": False,
                "error": f"Failed to create database '{db_name_raw}': {str(e)}"
            }
    ...
    def _drop_database(self, sql: str) -> dict:
        ...
        if not match:
            return {
                "success": False,
                "error": "Invalid DROP DATABASE syntax. Use: DROP DATABASE database_name"
            }
        ...
        if not db_file.exists() and not if_exists:
            return {
                "success": False,
                "error": f"Database '{db_name_raw}' does not exist"
            }
        
        if not db_file.exists() and if_exists:
            return {
                "success": True,
                "message": f"Database '{db_name_raw}' does not exist (IF EXISTS specified)"
            }
        ...
        try:
            ...
            return {
                "success": True,
                "message": f"Database '{db_name_raw}' dropped successfully"
            }
        except Exception as e:
            return {
                "success": False,
                "error": f"Failed to drop database '{db_name_raw}': {str(e)}"
            }
    ...
    def _use_database(self, sql: str) -> dict:
        ...
        if not match:
            return {
                "success": False,
                "error": "Invalid USE DATABASE syntax. Use: USE database_name or USE DATABASE database_name"
            }
        ...
        if not db_file.exists():
            return {
                "success": False,
                "error": f"Database '{db_name_raw}' does not exist. Create it first with: CREATE DATABASE {db_name_raw}"
            }
        
        self.current_database = db_name
        return {
            "success": True,
            "message": f"Using database '{db_name_raw}'",
            "database_path": str(db_file)
        }
